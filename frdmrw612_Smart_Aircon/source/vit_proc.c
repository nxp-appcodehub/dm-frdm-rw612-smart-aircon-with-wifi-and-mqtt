...
#elif defined(PLATFORM_RW610)
#define DEVICE_ID VIT_RW610

//#elif defined(PLATFORM_RW612)
...
#define VIT_OPERATING_MODE (VIT_WAKEWORD_ENABLE | VIT_SPEECH2INTENT_ENABLE)
#else
...
VIT_Aircon_Command_t aircon_cmd = {0};
uint16_t cmd_id = 0;

#if RECORD_MIC
#define TEST_BUFF_DURATION  3U
#define TEST_BUFF_FRAMES    (TEST_BUFF_DURATION * 1000U / DEMO_FRAME_MS)
#define RECORD_BYTE_DEPTH   2
#define FRAME_SIZE          (SAMPLES_PER_FRAME * RECORD_BYTE_DEPTH * (DEMO_CHANNEL_NUM + 1))
#define TEST_BUFF_SIZE      (SAMPLES_PER_FRAME * RECORD_BYTE_DEPTH * (DEMO_MAX_CHANNEL_NUM + 1) * TEST_BUFF_FRAMES)
#endif
...
#if RECORD_MIC
static int8_t record_mic_buff[TEST_BUFF_SIZE];
...
{
    /* Make sure the DWT trace function is enabled. */
...
int VIT_Initialize(void *arg)
{
    ...
    for (j = (PL_NR_MEMORY_REGIONS - 1); j >= 0; j--)
    {
        ...
        if (VITMemoryTable.Region[order[j]].Size != 0)
        {
            ...
            pMemory[j] = pvPortMalloc(VITMemoryTable.Region[order[j]].Size + MEMORY_ALIGNMENT);
            if (!pMemory[j])
            {
...
#if VIT_MEM_INFO
    float vit_model_size_kb = (float) sizeof(VIT_Model_en) / 1024.0f;
    float vit_model_ram_size = 0;

    PRINTF(" RAM Allocated = %.1f kB\r\n", total_size_allocated);
    if (MODEL_LOCATION == VIT_MODEL_IN_SLOW_MEM)
        PRINTF(" VIT_MODEL_IN_ROM");
    else if (MODEL_LOCATION == VIT_MODEL_IN_FAST_MEM)
        PRINTF(" VIT_MODEL_IN_RAM");

    PRINTF(" @ 0x%x", &VIT_Model_en);
    PRINTF(" Size = %.1f kB\r\n", vit_model_size_kb);

    if (((int) &VIT_Model_en[0] & 0xFF000000) == 0x08000000)
    {
        vit_model_ram_size = 0; // Model placed in flash
    }
    else if (((int) &VIT_Model_en[0] & 0xFF000000) == 0x20000000)
    {
...
#ifdef AFE_VOICESEEKER
    AFE_VoiceSeekerConfig.vsl_config.device_id = Device_RW610_CM33_NO_DSP;
    //AFE_VoiceSeekerConfig.vsl_config.num_mics = 2;
    int16_t VS_Status = AFE_VoiceSeeker_Init(&AFE_VoiceSeekerConfig, SAMPLES_PER_FRAME,
            NUMBER_OF_CHANNELS, VOICESEEKER_ENABLE);


    if (VS_Status != AFE_VOICESEEKER_SUCCESS)
    {
...
int VIT_Execute(void *arg, void *inputBuffer, int size)
{
    ...
    PL_UINT16 FrameSize = SAMPLES_PER_FRAME * DEMO_CHANNEL_NUM;

    if (size != SAMPLES_PER_FRAME * DEMO_CHANNEL_NUM * BYTE_DEPTH)
    {
    ...
    */
    ...
    if (NUMBER_OF_CHANNELS > _1CHAN)
    {
        ...
        DeInterleave(inputBuffer, (PL_INT16*) TempData, SAMPLES_PER_FRAME, DEMO_MAX_CHANNEL_NUM);
...
//         memcpy((void *)&record_mic_buff[AudioIoFrameCnt * FRAME_SIZE % TEST_BUFF_SIZE], (void *)VIT_InputData, FRAME_SIZE);
         ...
         PL_INT16 *InputTempRecordMic = (PL_INT16*) &record_mic_buff[AudioIoFrameCnt * SAMPLES_PER_FRAME * RECORD_BYTE_DEPTH % TEST_BUFF_SIZE];
//         PL_INT16 *InputTempRecordMic = (PL_INT16*) &record_mic_buff[AudioIoFrameCnt * FRAME_SIZE % TEST_BUFF_SIZE];
         PL_INT16 *pDataInput = (PL_INT16*) inputBuffer;

         for (PL_UINT16 i = 0; i < SAMPLES_PER_FRAME; i++)
         {
             InputTempRecordMic[i] = pDataInput[(i * NUMBER_OF_CHANNELS)];
//             InputTempRecordMic[i*5 + 0] = pDataInput[(i * NUMBER_OF_CHANNELS) + 0];
//             InputTempRecordMic[i*5 + 1] = pDataInput[(i * NUMBER_OF_CHANNELS) + 1];
//             InputTempRecordMic[i*5 + 2] = pDataInput[(i * NUMBER_OF_CHANNELS) + 2];
//             InputTempRecordMic[i*5 + 3] = pDataInput[(i * NUMBER_OF_CHANNELS) + 3];
//             InputTempRecordMic[i*5 + 4] = VIT_InputData[i];
         }
     }
     else if ( AudioIoFrameCnt * FRAME_SIZE == TEST_BUFF_SIZE )
     {
         ...
         PRINTF("[VIT] %d bytes @ 0x%x\r\n", TEST_BUFF_SIZE, &record_mic_buff);
     }
     AudioIoFrameCnt++;
#endif
    ...
    {
        /* VIT */
        arm_mean_q31((q31_t*)cycles, WINDOW_FRAMES, (q31_t*)&avg_cycles);
        arm_max_q31((q31_t*)cycles, WINDOW_FRAMES, (q31_t*)&max_cycles, &max_index);
        ...
        /* VoiceSeeker */
        arm_mean_q31((q31_t*)vs_cycles, WINDOW_FRAMES, (q31_t*)&avg_cycles);
        arm_max_q31((q31_t*)vs_cycles, WINDOW_FRAMES, (q31_t*)&max_cycles, &max_index);
...
#if MIC_DB
    {
        ...
        /* Get RMS value */
        arm_rms_f32(&float_inputBuffer[0], SAMPLES_PER_FRAME, &rms[DEMO_MAX_CHANNEL_NUM][mic_db_count % WINDOW_FRAMES]);
        ...
        if ((mic_db_count + 1) % WINDOW_FRAMES == 0)
        {
            ...
            arm_mean_f32(&rms[DEMO_MAX_CHANNEL_NUM][0], WINDOW_FRAMES, &avg_rms);
    ...
    for (uint8_t ch = 0; ch < DEMO_MAX_CHANNEL_NUM; ch++)
    {
        //float sum[DEMO_MAX_CHANNEL_NUM] = { 0 };
        ...
        if ((mic_db_count + 1) % WINDOW_FRAMES == 0)
        {
            ...
            if(ch < DEMO_MAX_CHANNEL_NUM-2)
            {
            	db_average = db_average + value_dB[ch];
            }
            if(isinf(db_average))
            {
            	Mic_On = false;
    ...
    db_average = db_average/DEMO_MAX_CHANNEL_NUM;
    mic_db_count++;
#endif
...
void VIT_Task(void *pvParameters)
{
    ...
    VIT_ReturnStatus_en VIT_state = VIT_SUCCESS;

    /* Call MIC_Init before VIT_Initialize to get number of mics to be used */
	MIC_Init();

	VIT_state = VIT_Initialize(NULL);
    if (VIT_state != VIT_SUCCESS)
    {
        PRINTF("[VIT] Initialize error: %d\r\n", VIT_state);
        vTaskSuspend(NULL);
    }
    ...
    while (1)
    {
        ...
        if (MIC_Read(buff, SAMPLES_PER_FRAME * DEMO_CHANNEL_NUM * BYTE_DEPTH) == 0)
        {
        	superbuff = buff;
            VIT_Execute(NULL, buff, SAMPLES_PER_FRAME * DEMO_CHANNEL_NUM * BYTE_DEPTH);
        }
        else
        {
