                                                    ...
                                                    void *userData)
{
    ...
    if (prvData->dataLen > 0)
    {
        ...
        LCDIC_SendDataArrayDMA(lcdic, lcdicHandle, &txXfer);
    }
    else
    {
...
status_t DBI_LCDIC_DMA_WriteCommandData(dbi_iface_t *dbiIface, uint32_t command, const void *data, uint32_t len_byte)
{
...
#if MCUX_DBI_IFACE_ENABLE_READ
status_t DBI_LCDIC_DMA_ReadData(dbi_iface_t *dbiIface, uint32_t command, void *data, uint32_t len_byte)
{
...
status_t DBI_LCDIC_DMA_WriteMemory(dbi_iface_t *dbiIface, uint32_t command, const void *data, uint32_t len_byte)
{
    status_t status;
    lcdic_tx_xfer_t txXfer;
