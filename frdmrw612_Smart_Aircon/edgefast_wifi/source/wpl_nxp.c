...
char *ssid_list[MAX_NETWORKS];                    // SSID list
...
static int WLP_process_results(unsigned int count)
{
    ...
    static char ssid_storage[MAX_NETWORKS][MAX_SSID_LENGTH]; // espacio para guardar cada SSID
    uint32_t cont = 0;
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        if (ret == WM_SUCCESS)
        {
            ...
            if(cont<MAX_NETWORKS) //Copying the networks
            {
        		strncpy(ssid_storage[cont], scan_result.ssid, MAX_SSID_LENGTH);
        		ssid_storage[cont][MAX_SSID_LENGTH - 1] = '\0';
        		ssid_list[cont] = ssid_storage[i];
        		cont++;
            }
            ret = snprintf(
