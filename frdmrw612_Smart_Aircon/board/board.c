...
__RAMFUNC(SRAM) status_t BOARD_InitPsRam(void)
{
    ...
    BOARD_SetFlexspiClock(FLEXSPI, 0U, 2U); /* 320M / 3 - 5 3  */
...
#endif
    config.ahbConfig.buffer[FSL_FEATURE_FLEXSPI_AHB_BUFFER_COUNT - 1].enablePrefetch = false;
    ...
    do
    {
        ...
        cacheCfg.policy[1] = kCACHE64_PolicyWriteBack; // psram
        cacheCfg.policy[2] = kCACHE64_PolicyNonCacheable;
